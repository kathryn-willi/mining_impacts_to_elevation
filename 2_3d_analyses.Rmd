...
post_mine_slope <- terrain(mines[[2]],opt='slope',unit='degrees')
plot(post_mine_slope)
...
```{r}
pre_mine_slope <- terrain(mines[[1]],opt='slope',unit='degrees')
slope_mines <- brick(pre_mine_slope, post_mine_slope)
slope_mines[[3]] <- mines[[1]] - mines[[2]]
names(slope_mines) <- c('Slope_pre_mining','Slope_post_mining','Slope_DoD')
plot(slope_mines)
...
```{r}
pre_matrix <- matrix(raster::extract(mines[[1]],
                    raster::extent(mines[[1]])),
                    nrow=ncol(mines[[1]]), ncol=nrow(mines[[1]]))
pre_matrix %>%
  sphere_shade(texture = 'desert') %>%
...
```{r}
post_matrix <- matrix(raster::extract(mines[[2]],
                    raster::extent(mines[[2]])),
                    nrow=ncol(mines[[2]]), ncol=nrow(mines[[2]]))
post_matrix %>%
  sphere_shade(texture = 'desert') %>%
